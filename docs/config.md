...
Matrix Builder deployments also support a deployment-owned `siteBinding`. It is normally generated by the container entrypoint from a site ID or homeserver hostname; see [Matrix Builder site binding](./matrix-builder-site-binding.md). Unlike ordinary options, the binding is re-applied after well-known configuration is loaded.
